 ...
 * @since 3.2
 */
class EqlQueryTransformer extends EqlQueryRenderer {
	...
	EqlQueryTransformer(Sort sort) {
		this(sort, false, null);
	}

	EqlQueryTransformer(boolean countQuery, @Nullable String countProjection) {
		this(Sort.unsorted(), countQuery, countProjection);
	}

	private EqlQueryTransformer(Sort sort, boolean countQuery, @Nullable String countProjection) {
